...
b = a.collapse(
    "T: minimum within years T: variance over years",
    within_years=cf.seasons(),
    weights=True,
)
print(b)
...
print(b.coordinate("T").bounds.datetime_array)
b = a.collapse(
    "T: standard_deviation within years",
    within_years=cf.seasons(),
    weights=True,
)
print(b)
...
lat = cf.DimensionCoordinate(
    data=cf.Data(numpy.arange(-90, 92.5, 2.5), "degrees_north")
)
lon = cf.DimensionCoordinate(
    data=cf.Data(numpy.arange(0, 360, 5.0), "degrees_east")
)
...
time.set_data(
    cf.Data(
        numpy.arange(0.5, 60, 1),
        units="days since 1860-01-01",
        calendar="360_day",
    )
)
time
...
new_z_p = cf.DimensionCoordinate(
