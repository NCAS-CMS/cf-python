...
def _formula_terms(standard_name):
    ...
    aux.long_name = (
        'Computed from parametric {} vertical coordinates'.format(
            standard_name
        )
    )
    ...
    if standard_name == 'atmosphere_ln_pressure_coordinate':
        ...
        a.set_data(data)
        bounds = cf.Bounds()
        data = cf.Data([[0.044995, 0.145905],
                        [0.145905, 0.246815],
                        [0.246815, 0.347725]])
        bounds.set_data(data)
        a.set_bounds(bounds)
        ...
        # coordinate_reference:
        coordref.coordinate_conversion.set_domain_ancillaries(
            {'zsurf1': zsurf1_key, 'a': a_key, 'b1': b1_key, 'b2': b2_key,
             'zsurf2': zsurf2_key, 'ztop': ztop_key}
        )
        field.set_construct(coordref)
        ...
        sigma.set_data(data)
        bounds = cf.Bounds()
        data = cf.Data([[0.10555556, 0.09444444],
        ...
        aux.set_data(data)
        bounds = cf.Bounds()
        data = cf.Data([[15.01701191, 23.42877638],
                        [23.42877638, 31.86034296],
                        [31.86034296, 40.31150319]], units='m')
        bounds.set_data(data)
        aux.set_bounds(bounds)
        ...
        aux.set_data(data)
        bounds = cf.Bounds()
        data = cf.Data([[600.90909091, 509.90909091],
        ...
        aux.set_data(data)
        bounds = cf.Bounds()
        data = cf.Data([[1.50000e-01, 1.35000e-01],
                        [1.35000e-01, 1.20000e-01],
                        [9.22880e+02, 9.32895e+02]],
                       units='m', dtype='f8')
        bounds.set_data(data)
        aux.set_bounds(bounds)
    ...
    def test_compute_vertical_coordinates(self):
        ...
        for standard_name in cf.formula_terms.FormulaTerms.standard_names:
            ...
            self.assertTrue(
                ...
                "{}, {}, {}\n{}\n{}".format(
                    standard_name, x.array, a.array,
                    x.bounds.array, a.bounds.array
                )
            )
